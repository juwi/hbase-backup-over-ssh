...
# Do an hbase backup to a remote server - optimized for multiple cron jobs
# hbaseexport.sh
# Copyright (C) 2013  Julian Wissmann <julianwissmann@gmail.com>
# Based on the excellent rsync-ssh backup script by Brett Alton <brett.jr.alton@gmail.com> 
...
{
	echo " !! $2, exiting..."
	echo " !! USAGE: $0 ['uninstall' | 'install' | 'run'] ['push' | 'pull'] [hbase_dir] [remote_user] [remote_host] [remote_dir] [remote_ssh_port] [table_name]"
	cleanup # cleanup hbase log files
	exit $1
}
...
THEDATESEC=`date '+%Y%m%d-%s'` # 20071010-1192044000
...
# cron
LOCAL_CRON_NAME="hbase-$KEY.cron"
...
if [ "$ACTION" == "run" ]; then
	...
	# run hadoop
	# hadoop jar $PATH_TO HBASE.jar export $TABLE $LOCAL_FILE
	# hadoop fs -cat $LOCAL_FILE | ssh $REMOTE_HOST "dd of=$TARGET_FILE"

	if [ "$METHOD" == "push" ]; then # local to remote
		if [[ `ssh -p $REMOTE_SSH_PORT -i $LOCAL_KEY_FILE $REMOTE_USER@$REMOTE_HOST "test -d $REMOTE_DIR"` ]]; then
			now=`date +"%m-%d-%Y"`
			hadoop jar $HBASE_PATH/hbase.jar export $TABLE /export/$TABLE.backup # to require timestamp in future version
			hadoop fs -cat /export/$TABLE.backup | ssh -p $REMOTE_SSH_PORT -i $LOCAL_KEY_FILE $REMOTE_USER@$REMOTE_HOST "dd of=$REMOTE_DIR/$TABLE.backup_$now"
			hadoop fs -rm /export/$TABLE.backup 
		else
			now=`date +"%m-%d-%Y"`
			hadoop jar $HBASE_PATH/hbase.jar export $TABLE /export/$TABLE.backup # no timestamp here
			hadoop fs -cat /export/$TABLE.backup | ssh -p $REMOTE_SSH_PORT -i $LOCAL_KEY_FILE $REMOTE_USER@$REMOTE_HOST "dd of=$REMOTE_DIR/$TABLE.backup_$now"
			hadoop fs -rm /export/$TABLE.backup 
		fi
...
# --- INSTALLING ---
	...
	# needs 8 parameters to continue
	if [ $# -ne 8 ]; then
